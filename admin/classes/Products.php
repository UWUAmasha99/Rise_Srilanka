...
class Products
{
	...
	public function getProducts(){
		$q = $this->con->query("SELECT p.product_id, p.product_cat, p.product_brand,p.product_title, p.product_price,
		 p.product_desc, p.product_image,p.product_keywords, c.cat_title, c.cat_id, b.brand_id,
		  b.brand_title FROM products p JOIN categories c ON c.cat_id = p.product_cat JOIN brands b ON b.brand_id = p.product_brand");
		$products = [];
	...
	public function addProduct($product_name,
		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png") {
			...
			if ($file['size'] > (1024 * 2)) {
				...
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/assets/prod_images/".$uniqueImageName)) {
					
					$q = $this->con->query("INSERT INTO `products`(`product_cat`, `product_brand`, `product_title`, `product_price`, `product_desc`, `product_image`, `product_keywords`) VALUES ('$category_id', '$brand_id', '$product_name',  '$product_price', '$product_desc', '$uniqueImageName', '$product_keywords')");
	...
	public function editProductWithImage($pid,
		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png") {
			...
			if ($file['size'] > (1024 * 2)) {
				...
				"/assets/prod_images/".$uniqueImageName)) {
